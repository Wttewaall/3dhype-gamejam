...
/// Key3=Value3
/// 
/// You can have empty lines (they are ignored) and comments are supported (; or #)
/// Key4=Value4 ; This is supposed to be a comment, but will be part of Value4
/// 
/// Whitespace is trimmed between the Key, equals sign and Value
/// 
...
public class IniFile {
    ...
    private readonly Regex _keyValueRegex = new Regex(@"\s*(?<Key>[^=;#\s]+)\s*=\s*(?<Value>.+?)(?=\s*$)");
    ...
    public bool Save(string filename)
    {
        ...
        try
        {
            // FIXME: File.WriteAllText(filename, sb.ToString());
            return true;
        } catch
        {
            return false;
